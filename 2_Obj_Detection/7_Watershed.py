...
plt.figure(),plt.imshow(coin),plt.axis("off"),plt.title("watershed")

# %% contour with morphology
coin = cv2.imread("coins.jpg")
...
# black color
coin_gray = cv2.cvtColor(coin_blur, cv2.COLOR_BGR2GRAY)
plt.figure(),plt.imshow(coin_gray, cmap="gray"),plt.axis("off"),plt.title("gray")
...
plt.figure(),plt.imshow(dist_transform, cmap="gray"),plt.axis("off"),plt.title("distance")

#small  pic
ret, sure_foreground = cv2.threshold(dist_transform,0.4*np.max(dist_transform),255,0)
plt.figure(),plt.imshow(sure_foreground, cmap="gray"),plt.axis("off"),plt.title("foreground")

# backfround
sure_background = cv2.dilate(opening,kernel,iterations=1)
sure_foreground= np.uint8(sure_foreground)
unknown = cv2.subtract(sure_background,sure_foreground)
plt.figure(),plt.imshow(unknown, cmap="gray"),plt.axis("off"),plt.title("background")

#connection
ret, marker = cv2.connectedComponents(sure_foreground)
marker = marker +1
marker[unknown ==255] =0
plt.figure(),plt.imshow(marker, cmap="gray"),plt.axis("off"),plt.title("connection")

#watershed
marker = cv2.watershed(coin,marker)
plt.figure(),plt.imshow(marker, cmap="gray"),plt.axis("off"),plt.title("watershedconnection")
